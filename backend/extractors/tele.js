...
const { agent } = require("../download");

async function extractTeleReceiptData(urlOrId) {
  if (!urlOrId) {
  ...
  function pick(labelRegex, key) {
    const node = $(`*:contains("${labelRegex}")`).first();
    if (node.length) {
      const td = node.next("td");
      if (td.length) {
        data[key] = td.text().trim();
      }
    }
  }

  // Use cheerio to find text nodes matching patterns
...
module.exports = { extractTeleReceiptData };
